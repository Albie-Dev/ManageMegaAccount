...
namespace MMA.Domain
{
    public class LinkHelper
    {
        ...
        public static Uri GenerateUrlWithQueryParams<T>(string endpoint, string routePath, T paramObj)
        {
            // Combine the endpoint and routePath
            Uri baseUri = new Uri(endpoint);
            Uri fullUri = new Uri(baseUri, routePath);

            // Create UriBuilder with the combined URL
            var uriBuilder = new UriBuilder(fullUri);
            var parameters = HttpUtility.ParseQueryString(string.Empty);

            // Use reflection to get properties of the object
            PropertyInfo[] properties = typeof(T).GetProperties();
            foreach (var property in properties)
            {
                var value = property.GetValue(paramObj)?.ToString();
                if (value != null)
                {
